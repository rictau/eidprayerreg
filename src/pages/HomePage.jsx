...
function HomePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(timeslotCollection, (timeslotSnapshot) => {
        ...
        const mergedData = initialGelombangSalatOptions.map((option) => {
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        setSelectedGelombangSalat(data.kloter);
        
        setEmailNotification({
          severity: "info",
          message: "Email ini sudah digunakan untuk registrasi. Silakan ubah data registrasi jika diperlukan. (このメールアドレスは登録に使用されています。必要に応じて登録データを変更してください。)"
        });
      } else {
        setExistingRegistration(null);
        setEmailNotification({
